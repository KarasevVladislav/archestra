...
function LlmProxies({ initialData }: { initialData?: LlmProxiesInitialData }) {
  ...
  const goToConnect = useCallback(
    (id: string, opts?: { fromTable?: boolean }) => {
      const params = new URLSearchParams({ tab: "proxy", id });
      if (opts?.fromTable) params.set("from", "table");
      ...
      cell: ({ row }) => {
        ...
        return (
          <LlmProxyActions
            agent={agent}
            canModify={canModify}
            onConnect={(a) => goToConnect(a.id, { fromTable: true })}
            onEdit={(agentData) => {
