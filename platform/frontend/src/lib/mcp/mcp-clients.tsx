import type { ReactNode } from "react";

export type McpClientAuthMode = "oauth" | "token";

export interface McpClientBuildArgs {
  mcpUrl: string;
  /** Token value (may be masked for display); null when using OAuth */
  token: string | null;
  serverName: string;
}

export interface McpClientConfigOutput {
  code: string;
  language: "json" | "bash" | "toml" | "text";
}

export interface McpClientStep {
  title: string;
  body: string;
}

export interface McpClientQuickInstall {
  label: string;
  buildHref: (args: McpClientBuildArgs) => string;
}

/**
 * Describes one MCP client. Register a client here and it automatically
 * appears in the connection page picker with the right capabilities.
 *
 * Clients can mix and match capabilities:
 *  - auth.oauth / auth.token — which auth modes show up in the toggle
 *  - quickInstall — deep-link one-click install button
 *  - buildConfig — snippet shown in the code block (JSON/TOML/bash)
 *  - steps — manual setup checklist
 */
export interface McpClientDefinition {
  id: string;
  label: string;
  sub: string;
  icon: ReactNode;
  tileBg?: string;
  configFile?: string;
  tip?: string;
  auth: {
    oauth: boolean;
    token: boolean;
  };
  quickInstall?: McpClientQuickInstall;
  /** Default steps. Used when no auth-specific override is set. */
  steps: McpClientStep[];
  /** Optional OAuth-specific steps. When set and OAuth is selected, these replace `steps`. */
  oauthSteps?: McpClientStep[];
  /** Optional token-specific steps. When set and static token is selected, these replace `steps`. */
  tokenSteps?: McpClientStep[];
  buildConfig?: (args: McpClientBuildArgs) => McpClientConfigOutput;
}

export function getStepsForAuthMode(
  client: McpClientDefinition,
  authMode: McpClientAuthMode,
): McpClientStep[] {
  if (authMode === "oauth" && client.oauthSteps) return client.oauthSteps;
  if (authMode === "token" && client.tokenSteps) return client.tokenSteps;
  return client.steps;
}

function ClaudeIcon({ size = 20 }: { size?: number }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} aria-hidden="true">
      <path
        d="m4.7144 15.9555 4.7174-2.6471.079-.2307-.079-.1275h-.2307l-.7893-.0486-2.6956-.0729-2.3375-.0971-2.2646-.1214-.5707-.1215-.5343-.7042.0546-.3522.4797-.3218.686.0608 1.5179.1032 2.2767.1578 1.6514.0972 2.4468.255h.3886l.0546-.1579-.1336-.0971-.1032-.0972L6.973 9.8356l-2.55-1.6879-1.3356-.9714-.7225-.4918-.3643-.4614-.1578-1.0078.6557-.7225.8803.0607.2246.0607.8925.686 1.9064 1.4754 2.4893 1.8336.3643.3035.1457-.1032.0182-.0728-.164-.2733-1.3539-2.4467-1.445-2.4893-.6435-1.032-.17-.6194c-.0607-.255-.1032-.4674-.1032-.7285L6.287.1335 6.6997 0l.9957.1336.419.3642.6192 1.4147 1.0018 2.2282 1.5543 3.0296.4553.8985.2429.8318.091.255h.1579v-.1457l.1275-1.706.2368-2.0947.2307-2.6957.0789-.7589.3764-.9107.7468-.4918.5828.2793.4797.686-.0668.4433-.2853 1.8517-.5586 2.9021-.3643 1.9429h.2125l.2429-.2429.9835-1.3053 1.6514-2.0643.7286-.8196.85-.9046.5464-.4311h1.0321l.759 1.1293-.34 1.1657-1.0625 1.3478-.8804 1.1414-1.2628 1.7-.7893 1.36.0729.1093.1882-.0183 2.8535-.607 1.5421-.2794 1.8396-.3157.8318.3886.091.3946-.3278.8075-1.967.4857-2.3072.4614-3.4364.8136-.0425.0304.0486.0607 1.5482.1457.6618.0364h1.621l3.0175.2247.7892.522.4736.6376-.079.4857-1.2142.6193-1.6393-.3886-3.825-.9107-1.3113-.3279h-.1822v.1093l1.0929 1.0686 2.0035 1.8092 2.5075 2.3314.1275.5768-.3218.4554-.34-.0486-2.2039-1.6575-.85-.7468-1.9246-1.621h-.1275v.17l.4432.6496 2.3436 3.5214.1214 1.0807-.17.3521-.6071.2125-.6679-.1214-1.3721-1.9246L14.38 17.959l-1.1414-1.9428-.1397.079-.674 7.2552-.3156.3703-.7286.2793-.6071-.4614-.3218-.7468.3218-1.4753.3886-1.9246.3157-1.53.2853-1.9004.17-.6314-.0121-.0425-.1397.0182-1.4328 1.9672-2.1796 2.9446-1.7243 1.8456-.4128.164-.7164-.3704.0667-.6618.4008-.5889 2.386-3.0357 1.4389-1.882.929-1.0868-.0062-.1579h-.0546l-6.3385 4.1164-1.1293.1457-.4857-.4554.0608-.7467.2307-.2429 1.9064-1.3114Z"
        fill="#D97757"
      />
    </svg>
  );
}

function CursorIcon({ size = 20 }: { size?: number }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} aria-hidden="true">
      <path
        d="M11.503.131 1.891 5.678a.84.84 0 0 0-.42.726v11.188c0 .3.162.575.42.724l9.609 5.55a1 1 0 0 0 .998 0l9.61-5.55a.84.84 0 0 0 .42-.724V6.404a.84.84 0 0 0-.42-.726L12.497.131a1.01 1.01 0 0 0-.996 0M2.657 6.338h18.55c.263 0 .43.287.297.515L12.23 22.918c-.062.107-.229.064-.229-.06V12.335a.59.59 0 0 0-.295-.51l-9.11-5.257c-.109-.063-.064-.23.061-.23"
        fill="currentColor"
      />
    </svg>
  );
}

function McpIcon({ size = 20 }: { size?: number }) {
  return (
    <svg
      viewBox="0 0 180 180"
      width={size}
      height={size}
      fill="none"
      aria-hidden="true"
    >
      <path
        d="M23.5996 85.2532L86.2021 22.6507C94.8457 14.0071 108.86 14.0071 117.503 22.6507C126.147 31.2942 126.147 45.3083 117.503 53.9519L70.2254 101.23"
        stroke="currentColor"
        strokeWidth={11.0667}
        strokeLinecap="round"
      />
      <path
        d="M70.8789 100.578L117.504 53.952C126.148 45.3083 140.163 45.3083 148.806 53.952L149.132 54.278C157.776 62.9216 157.776 76.9357 149.132 85.5792L92.5139 142.198C89.6327 145.079 89.6327 149.75 92.5139 152.631L104.14 164.257"
        stroke="currentColor"
        strokeWidth={11.0667}
        strokeLinecap="round"
      />
      <path
        d="M101.853 38.3013L55.553 84.6011C46.9094 93.2447 46.9094 107.258 55.553 115.902C64.1966 124.546 78.2106 124.546 86.8543 115.902L133.154 69.6025"
        stroke="currentColor"
        strokeWidth={11.0667}
        strokeLinecap="round"
      />
    </svg>
  );
}

function CodexIcon({ size = 20 }: { size?: number }) {
  return (
    <svg viewBox="0 0 24 24" width={size} height={size} aria-hidden="true">
      <path
        d="M22.282 9.821a5.985 5.985 0 0 0-.516-4.91 6.046 6.046 0 0 0-6.51-2.9A6.065 6.065 0 0 0 4.981 4.18a5.985 5.985 0 0 0-3.998 2.9 6.046 6.046 0 0 0 .743 7.097 5.98 5.98 0 0 0 .51 4.911 6.051 6.051 0 0 0 6.515 2.9A5.985 5.985 0 0 0 13.26 24a6.056 6.056 0 0 0 5.772-4.206 5.99 5.99 0 0 0 3.997-2.9 6.056 6.056 0 0 0-.747-7.073zM13.26 22.43a4.476 4.476 0 0 1-2.876-1.04l.141-.081 4.779-2.758a.795.795 0 0 0 .392-.681v-6.737l2.02 1.168a.071.071 0 0 1 .038.052v5.583a4.504 4.504 0 0 1-4.494 4.494z"
        fill="currentColor"
      />
    </svg>
  );
}

function buildJsonConfig({
  mcpUrl,
  token,
  serverName,
}: McpClientBuildArgs): McpClientConfigOutput {
  const entry: { url: string; headers?: Record<string, string> } = {
    url: mcpUrl,
  };
  if (token) entry.headers = { Authorization: `Bearer ${token}` };
  return {
    language: "json",
    code: JSON.stringify({ mcpServers: { [serverName]: entry } }, null, 2),
  };
}

export const MCP_CLIENTS: McpClientDefinition[] = [
  {
    id: "claude-code",
    label: "Claude Code",
    sub: "Anthropic CLI",
    icon: <ClaudeIcon />,
    tileBg: "#fff1ea",
    configFile: "terminal",
    tip: "Paste it into your terminal — one command.",
    auth: { oauth: true, token: true },
    steps: [
      {
        title: "Add the gateway",
        body: "Run the generated command. Claude Code stores it under ~/.claude.json.",
      },
      {
        title: "Verify",
        body: "Type /mcp inside Claude Code to confirm the gateway is online.",
      },
    ],
    oauthSteps: [
      {
        title: "Add the gateway",
        body: "Run the generated command in your terminal.",
      },
      {
        title: "Authenticate via /mcp",
        body: "Inside Claude Code, run /mcp, pick this gateway, and choose Authenticate. Complete the browser sign-in to finish OAuth.",
      },
      {
        title: "Verify",
        body: "Run /mcp again — the server should show as connected.",
      },
    ],
    tokenSteps: [
      {
        title: "Add the gateway",
        body: "Run the generated command. Claude Code stores it under ~/.claude.json.",
      },
      {
        title: "Verify",
        body: "Type /mcp inside Claude Code to confirm the gateway is online.",
      },
    ],
    buildConfig: ({ mcpUrl, token, serverName }) => {
      if (!token) {
        return {
          language: "bash",
          code: `claude mcp add --transport http ${serverName} ${mcpUrl}`,
        };
      }
      return {
        language: "bash",
        code: `claude mcp add --transport http \\\n    --header "Authorization: Bearer ${token}" \\\n    ${serverName} \\\n    ${mcpUrl}`,
      };
    },
  },
  {
    id: "cursor",
    label: "Cursor",
    sub: "AI code editor",
    icon: <CursorIcon />,
    tileBg: "#fafaff",
    configFile: "~/.cursor/mcp.json",
    tip: "Or follow the manual steps to paste the config into mcp.json.",
    auth: { oauth: true, token: true },
    quickInstall: {
      label: "Open in Cursor",
      buildHref: ({ mcpUrl, serverName }) => {
        const payload = btoa(JSON.stringify({ url: mcpUrl }));
        return `cursor://anysphere.cursor-deeplink/mcp/install?name=${encodeURIComponent(serverName)}&config=${payload}`;
      },
    },
    steps: [
      {
        title: "Open Cursor settings",
        body: "Cmd ⌘ + , → MCP → Edit mcp.json.",
      },
      {
        title: "Paste the config",
        body: "Drop the snippet into your mcpServers block and save.",
      },
      {
        title: "Enable the server",
        body: "Toggle the server on under MCP Servers. Tools appear in the @-mention menu.",
      },
    ],
    oauthSteps: [
      {
        title: "Open Cursor settings",
        body: "Cmd ⌘ + , → MCP → Edit mcp.json.",
      },
      {
        title: "Paste the config",
        body: "Drop the snippet into your mcpServers block and save.",
      },
      {
        title: "Sign in through the browser",
        body: "Cursor shows the server as 'Needs login' under MCP Servers. Click it to launch the OAuth consent flow and authorize access.",
      },
    ],
    tokenSteps: [
      {
        title: "Open Cursor settings",
        body: "Cmd ⌘ + , → MCP → Edit mcp.json.",
      },
      {
        title: "Paste the config",
        body: "Drop the snippet into your mcpServers block and save.",
      },
      {
        title: "Enable the server",
        body: "Toggle the server on under MCP Servers. Tools appear in the @-mention menu.",
      },
    ],
    buildConfig: buildJsonConfig,
  },
  {
    id: "codex",
    label: "Codex",
    sub: "OpenAI coding agent",
    icon: <CodexIcon />,
    tileBg: "#fafaff",
    configFile: "terminal",
    tip: "Paste it into your terminal — one command.",
    auth: { oauth: true, token: true },
    steps: [
      {
        title: "Add the gateway",
        body: "Run the generated command. Codex stores it under ~/.codex/config.toml.",
      },
      {
        title: "Reload Codex",
        body: "Start a new conversation. Codex discovers tools on first tool call.",
      },
    ],
    oauthSteps: [
      {
        title: "Add the gateway",
        body: "Run the generated command in your terminal.",
      },
      {
        title: "Log in",
        body: "Run 'codex mcp login <server>' to complete OAuth consent in your browser. Codex persists and refreshes tokens automatically.",
      },
      {
        title: "Reload Codex",
        body: "Start a new conversation — the server's tools are now available.",
      },
    ],
    tokenSteps: [
      {
        title: "Export your token",
        body: "Put the token in an environment variable (e.g. ARCHESTRA_MCP_TOKEN=<value>) so Codex can read it at runtime.",
      },
      {
        title: "Add the gateway",
        body: "Run the generated command. Codex stores the server definition under ~/.codex/config.toml.",
      },
      {
        title: "Reload Codex",
        body: "Start a new conversation. Codex discovers tools on first tool call.",
      },
    ],
    buildConfig: ({ mcpUrl, token, serverName }) => {
      if (!token) {
        return {
          language: "bash",
          code: `codex mcp add ${serverName} --url ${mcpUrl}`,
        };
      }
      return {
        language: "bash",
        code: `codex mcp add ${serverName} \\\n    --url ${mcpUrl} \\\n    --bearer-token-env-var ARCHESTRA_MCP_TOKEN`,
      };
    },
  },
  {
    id: "generic",
    label: "Generic",
    sub: "Anything MCP-compatible",
    icon: <McpIcon />,
    tileBg: "#f1f1fa",
    configFile: undefined,
    tip: "Point any MCP-capable client at this URL.",
    auth: { oauth: true, token: true },
    steps: [],
  },
];

export function getMcpClient(id: string): McpClientDefinition | undefined {
  return MCP_CLIENTS.find((c) => c.id === id);
}
