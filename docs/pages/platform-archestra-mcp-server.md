...
| `convert_conversation_to_scheduled_task` | Create a recurring scheduled task from the current conversation. | `scheduledTask:create` |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `cron_expression` | `string` | Yes | 5-field cron expression matching the in-app schedule form. Allowed forms: 'M * * * *' (hourly, M=0-59) or 'M H * * D' (daily, H=0-23, D='*' or comma-separated weekdays 0-6 with Sun=0). Examples: '0 * * * *', '0 9 * * *', '30 14 * * 1,2,3,4,5'. Day-of-month, month, ranges (1-5), and steps (*/15) are not supported. |
| `timezone` | `string` | Yes | IANA timezone identifier for the schedule, e.g. 'UTC' or 'Europe/London'. |
| `name` | `string` | No | Optional human-readable name for the scheduled task. Defaults to the conversation title or a snippet of the first user message. |
| `message_template` | `string` | No | Optional standalone prompt to run on every schedule tick. When omitted, the conversation is summarized into a prompt automatically. |
| `agent_name` | `string` | No | Optional exact agent name to run the scheduled task. When omitted, the most recently used agent in this conversation is selected. When reply_in_same_conversation is true, this must be the current chat agent or one that has previously participated in the conversation. |
| `enabled` | `boolean` | No | Whether the scheduled task should start enabled. Defaults to true. |
| `reply_in_same_conversation` | `boolean` | No | When true, each successful run appends the prompt and assistant reply to this conversation instead of an isolated run thread. If the conversation's agent later changes (e.g. swap_agent) and the scheduled task's agent is no longer valid for the chat, the task auto-unlinks from the conversation and continues as standalone runs. |

##### Output

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `success` | `boolean` | Yes | Whether the conversion succeeded. |
| `trigger_id` | `string` | Yes | The ID of the created scheduled task (trigger). |
| `agent_id` | `string` | Yes | The ID of the agent that was selected for the scheduled task. |
| `name` | `string` | Yes | The name of the created scheduled task. |
| `cron_expression` | `string` | Yes | The cron expression stored for the scheduled task. |
